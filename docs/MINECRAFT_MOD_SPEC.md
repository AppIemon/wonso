...
**wonso** Fabric 모드는 Minecraft 야생에 **118원소 화학 시스템**을 추가한다. 채굴·탐험으로 원소를 모으고, 합성실에서 분자를 만들고, 주기율표 GUI로 원소·원자·분자를 학습한다. elementwar.xyz의 카드·합성·에너지·도감 시스템을 서바이벌 맥락에 이식한다.
...
| **멀티 친화** | 싱글·서버 모두 동작, PvP는 후순위 |

### 2.1 야생 진행 곡선

```text
Day 1~3   바닐라 기본 + Chemistry Journal 획득
Day 3~7   첫 원소 카드 (H, C, O, Fe, Cu) + 기본 합성실
Day 7~14  분자 도감 시작, 에너지 시스템, Tier 1 원소 (Z 1~20)
Day 14~30 전이금속, 네더 원소 (S, Na, Cl), 핵융합 입문
Day 30+   희귀 원소, 별 시스템, 엔드·Og 추적, 라운드 모드
```
...
| 획득 | 광물 정제, 바닐라 아이템 분해, 몹 드롭, 구조물 |
| 속성 | attack, health, rarity, cost, category(족) |
| 용도 | 합성 재료, 전투 배치(PvE), 족 시너지, 도감 |
